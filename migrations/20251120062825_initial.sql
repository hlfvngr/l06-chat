...
CREATE TABLE IF NOT EXISTS chats (
    id bigint PRIMARY KEY AUTO_INCREMENT not null comment '聊天ID',
...
CREATE TABLE IF NOT EXISTS messages (
    id bigint PRIMARY KEY AUTO_INCREMENT not null comment '消息ID',
    chat_id bigint NOT NULL comment '聊天ID',
...
CREATE TABLE IF NOT EXISTS workspaces (
    id bigint PRIMARY KEY AUTO_INCREMENT not null comment '空间ID',
...
CREATE TABLE IF NOT EXISTS outbox_messages (
    ...
    last_retry_time DATETIME comment '上次重试时间',
    send_fail_reason TEXT comment '发送失败原因',
    next_retry_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP() comment '下次重试时间',
    send_success_time DATETIME comment '发送成功时间'
) comment '出站消息表';

create index idx_outbox_messages_next_retry_time on outbox_messages (next_retry_time, send_status);
